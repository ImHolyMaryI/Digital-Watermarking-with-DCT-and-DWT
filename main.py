...
def convert_image(image_name: str, size: int) -> np.ndarray:
    ...
    image_array = np.array(img.getdata(), dtype=np.uintc).reshape((size, 
size))
...
class DigitalWatermarkDctDwt:
    def __init__(self, image_name: str, watermark_name: str, model: str = 
"haar", level: int = 1):
        self.image_array: np.ndarray = convert_image(image_name, 2048)
        self.watermark_array: np.ndarray = convert_image(watermark_name, 
128)
    ...
    def __process_coefficients(self, im_array: np.ndarray) -> list:
        ...
        """
        coeffs = pywt.wavedec2(data=im_array, wavelet=self.model, 
level=self.level)
    ...
    def __apply_dct(image_array: list) -> np.ndarray:
        ...
        for i in range(0, size, 8):
            for j in range(0, size, 8):
                ...
                subdct = dct(dct(subpixels.T, norm="ortho").T, 
norm="ortho")
    ...
    def __inverse_dct(self) -> np.ndarray:
        ...
        for i in range(0, size, 8):
            for j in range(0, size, 8):
                subidct = \
                    idct(idct(self.dct_array[i:i + 8, j:j + 8].T, 
norm="ortho").T, norm="ortho")
    ...
    @staticmethod
    def __get_watermark(dct_watermarked_coeff: np.ndarray, watermark_size: 
int) -> np.ndarray:
        subwatermarks = []
        ...
        watermark = np.array(subwatermarks).reshape(watermark_size, 
watermark_size)

        return watermark

    def __recover_watermark(self) -> None:
        coeffs_watermarked_image = 
self.__process_coefficients(self.image_array_H)
        dct_watermarked_coeff = 
self.__apply_dct(coeffs_watermarked_image[0])
    ...
    def implementation_dw(self):
        ...
        self.dct_array: np.ndarray = 
self.__apply_dct(self.coeffs_image[0])
        self.dct_array = self.__embed_watermark()
        self.coeffs_image[0]: np.ndarray = self.__inverse_dct()

    def recover_watermark(self):
        self.image_array_H: np.ndarray = pywt.waverec2(self.coeffs_image, 
self.model)
